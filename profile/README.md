<p align="center">
  <a href="#">
      <picture>
        <source media="(prefers-color-scheme: dark)" srcset="../branding/webkernel-logo-icon-dark.png">
        <img alt="Webkernel" src="../branding/webkernel-logo-icon-light.png">
      </picture>
    </a>
</p>
  
<h1 align="center">Webkernel</h1>
<div align="center">
Webkernel is an open application engine designed to operate self-contained modules. 
  <br /><br />
  It combines a ready-to-use backoffice, a unified data layer, and an instant browser setup into a single environment where features plug in like apps.
	<br />
	<br />
	<sub>
		Check out the <a href="https://webkernelphp.com/docs">documentation</a>
		&nbsp;
  </sub>
</p>
